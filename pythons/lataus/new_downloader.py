...
class Downloader:
  def __init__(self, download_destination, error_file_destination, progress_file_destination, exists_file_destination, verbose=False) -> None:
    ...
    self.existing_files = []
    self.ely_codes = ['01', '02', '03', '04', '08', '09', '10', '12', '14']

  def download_lam_data(self, year, ely, file_name, destination):
    ely_index = self.ely_codes.index(ely)
    if ely_index < len(self.existing_files):
      exists =  file_name in self.existing_files[ely_index]
      if exists:
        self.existed_queue.put(file_name)
        return
  ...
  def download(self, download_args_list, file_identifier=""):
    ...
    fwt = FileWritterThread(self.done_queue, self.progress_file_destination, self.error_queue, self.error_file_destination, self.existed_queue, self.exists_file_destination, file_identifier)
    fwt.start()

    single_args = download_args_list[0]
    ...
    for directory in os.listdir(data_parent_dir):
      self.existing_files.append(set(os.listdir(f"{data_parent_dir}\\{directory}")))
...
class FileWritterThread(threading.Thread):
  def __init__(self, doneq, donef, errorq, errorf, existq, existf, file_identifier):
    ...
    try:
      while self.tasks_left:
        if self.done_queue.qsize() + self.existed_queue.qsize() + self.error_queue.qsize() == 0:
          ...
          time.sleep(5)
          print(f"Queued rows {(self.done_queue.qsize(), self.existed_queue.qsize(), self.error_queue.qsize())}")
