...
def create_mapbox_with_selected_highlighted(ids, circle_rad=5):
    # Must be a public token, starting with `pk`
    access_token = 'YOUR_TOKEN_HERE'
    ...
    for f in feats:
        feat_tms_id = f["properties"]["tmsNumber"]
        
        should_highlight = feat_tms_id in ids

        f["properties"]["color"] = highlight_color if should_highlight else disabled_color

    main_source = GeoJSONSource(stations, source_id='points1')

    paint = Paint(circle_radius=circle_rad, circle_color=['get', 'color'])
